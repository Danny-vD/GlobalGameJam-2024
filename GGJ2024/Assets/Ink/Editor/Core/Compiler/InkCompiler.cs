...
namespace Ink.UnityIntegration
{
	...
	public class InkCompiler : ScriptableObject {
        ...
        public static bool executingCompilationStack => instance.compilationStack.Count > 0;
        ...
        public static void CompileInk(InkFile[] inkFiles, bool immediate, Action onComplete = null)
        {
            ...
            var filesCompiledLog = new StringBuilder("Files compiled:");
            ...
            foreach (var inkFile in inkFiles) CompileInkInternal(inkFile, immediate);
        }
        ...
        {
            var masterInkFiles = new List<InkFile>();
            ...
            foreach (var masterInkFile in GetMasterFilesIncludingInkAssetPath(importedAssetPath))
                if (!masterInkFiles.Contains(masterInkFile) &&
                    InkSettings.instance.ShouldCompileInkFileAutomatically(masterInkFile))
        ...
        private static bool playModeBlocked;


        // Track if we've currently locked compilation of Unity C# Scripts
        private static bool hasLockedUnityCompilation;
        ...
        [SerializeField] private List<string> pendingCompilationStack = new();

        // The state of files currently being compiled.
        [SerializeField] private List<CompilationStackItem> compilationStack = new();
            ...
            public void SetOutputLog()
            {
                ...
                foreach (var output in logOutput)
                    if (output.type == ErrorType.Error)
                    {
                        ...
                        Debug.LogError(
                            "Ink " + output.type + " for " + Path.GetFileName(inkFile.filePath) + ": " +
                            output.content + " (at " + output.relativeFilePath + ":" + output.lineNumber + ")",
                            inkFile.inkAsset);
                    }
                    ...
                    else if (output.type == ErrorType.Author)
                    {
                        ...
                        if (InkSettings.instance.printInkLogsInConsoleOnCompile)
                            Debug.Log(
                                "Ink Log for " + Path.GetFileName(inkFile.filePath) + ": " + output.content + " (at " +
                                output.relativeFilePath + " " + output.lineNumber + ")", inkFile.inkAsset);
                    }
            }
        }
        ...
        private class AssetSaver : AssetModificationProcessor
        {
        ...
        private static void Update()
        {
            ...
            if (executingCompilationStack)
            {
                ...
                for (var i = instance.compilationStack.Count - 1; i >= 0; i--)
                {
        ...
        private static void CompileInkInternal(InkFile inkFile, bool immediate)
        {
            ...
            var inputPath = InkEditorUtils.CombinePaths(inkFile.absoluteFolderPath, Path.GetFileName(inkFile.filePath));
            Debug.Assert(inkFile.absoluteFilePath == inputPath);

            var pendingFile = new CompilationStackItem
            {
        ...
        private static void BeginCompilingFile(CompilationStackItem item)
        {
            ...
            {
                Debug.LogError("Cannot begin compiling ink file because it is " + item.state +
                               "! This is most likely a threading bug. Please report this!");
                return;
            }
            ...
            Progress.Report(compileProgressID, instance.compilationStack.IndexOf(item),
        ...
        private static void CompleteCompilingFile(CompilationStackItem item)
        {
            ...
            {
                Debug.LogError("Cannot complete compiling ink file because it is " + item.state +
                               "! This is most likely a threading bug. Please report this!");
                return;
            }
            ...
            if (!AnyOfStateInCompilationStack(CompilationStackItemState.Compiling) &&
                !AnyOfStateInCompilationStack(CompilationStackItemState.Queued))
                OnCompleteCompilationStack();
        }
        ...
        {
            var item = (CompilationStackItem)itemObj;
            if (item.state != CompilationStackItemState.Compiling)
                Debug.LogError("File passed to CompileInkThreaded is not in Compiling state.");
            ...
            var compiler = new Compiler(inputString, new Compiler.Options
            {
                ...
                errorHandler = (message, type) =>
                {
                    ...
                    {
                        if (string.IsNullOrEmpty(log.relativeFilePath))
                            log.relativeFilePath = Path.GetFileName(item.inkAbsoluteFilePath);
        ...
        private static void OnCompleteCompilationStack()
        {
            ...
            var compilationStack = new List<CompilationStackItem>(instance.compilationStack);
            ...
            foreach (var compilingFile in compilationStack)
                if (compilingFile.inkFile.inkAsset != null)
                {
                    ...
                    if (compilingFile.unhandledErrorOutput.Count > 0)
                    {
                        ...
                        errorLog.Append("'");
                        errorLog.AppendLine(
                            "! You may be able to resolve your issue by using Edit > Rebuild Ink Library.");
                    ...
                    else
                    {
                        ...
                        var errorsInEntireStory = false;
                        var warningsInEntireStory = false;
                        foreach (var inkFile in compilingFile.inkFile.includesInkFiles)
                        {
                            if (inkFile.hasErrors) errorsInEntireStory = true;
                            if (inkFile.hasWarnings) warningsInEntireStory = true;
                        }
                        ...
                        if (warningsInEntireStory) filesCompiledLog.Append(" (With warning)");
                    }
                }
                else
                {
                    ...
                    Debug.LogError("Ink file at " + compilingFile.inkAbsoluteFilePath +
                                   " was not found after compilation");
                }
            ...
            {
                var inkFilesCompiled = new InkFile[compilationStack.Count];
                for (var i = 0; i < compilationStack.Count; i++) inkFilesCompiled[i] = compilationStack[i].inkFile;
        ...
        {
            var numCompiling = CountOfStateInCompilationStack(CompilationStackItemState.Compiling);
            var compilingFile = GetCurrentlyCompilingFile();
            var message = "";
            if (compilingFile == null) message += "Compiling...";
            else
                message += "Compiling " + compilingFile.inkFile.inkAsset.name + " for " + compilingFile.timeTaken + "s";
            message += "\n.Ink File " + (instance.compilationStack.Count - numCompiling) + " of " +
                       instance.compilationStack.Count + ".";
