...
namespace Ink.UnityIntegration
{
    ...
    public static class InkEditorUtils
    {
        ...
        public static Texture2D inkLogoIcon
        {
            get
            {
                if (_inkLogoIcon == null) _inkLogoIcon = Resources.Load<Texture2D>("InkLogoIcon");
                return _inkLogoIcon;
            }
        }
        ...
        public static void CreateNewInkFileAtPathWithTemplateAndStartNameEditing(string filePath,
        ...
        ///     TODO - If the editor is inky, this code should load the master file, but immediately show the correct child file at
        ///     the correct line.
        /// </summary>
        ...
        public static void OpenInEditor(string masterFilePath, string subFilePath, int lineNumber)
        {
            ...
            {
                Debug.LogWarning("Tried to open an ink file (" + subFilePath + ") at line " + lineNumber +
                                 " but the file is an include file. This is not currently implemented. The master ink file will be opened at line 0 instead.");
                lineNumber = 0;
            }
#if UNITY_2019_1_OR_NEWER
            // This function replaces OpenFileAtLineExternal, but I guess it's totally internal and can't be accessed.
...
#pragma warning restore
        ...
        public static string FindAbsolutePluginDirectory()
        {
            ...
            var guids = AssetDatabase.FindAssets("t:DefaultAsset", new[] { "Assets" })
                .Where(g => AssetDatabase.GUIDToAssetPath(g).EndsWith("/InkLibs")).ToArray();
            if (guids.Length > 0)
            {
                var assetPathOfInkLibsFolder = AssetDatabase.GUIDToAssetPath(guids[0]);
                var rootPluginFolder =
                    assetPathOfInkLibsFolder.Substring(0, assetPathOfInkLibsFolder.Length - "/InkLibs".Length);
