...
namespace Ink.UnityIntegration
{
    public class InkInspector : DefaultAssetInspector
    {
        ...
        public override void OnHeaderGUI()
        {
            ...
            var rect = new Rect(lastRect.x, lastRect.y, lastRect.width, lastRect.height);
            var iconRect = new Rect(rect.x + 6f, rect.y + 6f, 32f, 32f);
            GUI.DrawTexture(iconRect, InkBrowserIcons.inkFileIconLarge);
            var childIconRect = new Rect(iconRect.x, iconRect.y, 16f, 16f);
            if (inkFile == null)
            ...
            var titleRect = new Rect(rect.x + 44f, rect.y + 6f, rect.width - 44f - 38f - 4f, 16f);
            titleRect.yMin -= 2f;
            titleRect.yMax += 2f;
        ...
        {
            var includeTextAssets = inkFile.includes;
            includesFileList = new ReorderableList(includeTextAssets, typeof(DefaultAsset), false, true, false, false);
            includesFileList.drawHeaderCallback = rect => { EditorGUI.LabelField(rect, "Included Files"); };
            includesFileList.drawElementCallback = (rect, index, isActive, isFocused) =>
            {
                var childAssetFile = ((List<DefaultAsset>)includesFileList.list)[index];
                ...
                var childInkFile = InkLibrary.GetInkFileWithFile(childAssetFile);
                ...
                var iconRect = new Rect(rect.x, rect.y, 0, 16);
                if (childInkFile.hasErrors || childInkFile.hasWarnings) iconRect.width = 20;
                var objectFieldRect = new Rect(iconRect.xMax, rect.y, rect.width - iconRect.width - 80, 16);
                var selectRect = new Rect(objectFieldRect.xMax, rect.y, 80, 16);
                ...
                EditorGUI.EndDisabledGroup();
                if (GUI.Button(selectRect, "Select")) Selection.activeObject = childAssetFile;
            };
        }

        private void CreateMastersList()
        {
            var mastersTextAssets = inkFile.masterInkAssets;
            mastersFileList = new ReorderableList(mastersTextAssets, typeof(DefaultAsset), false, true, false, false);
            mastersFileList.drawHeaderCallback = rect => { EditorGUI.LabelField(rect, "Master Files"); };
            mastersFileList.drawElementCallback = (rect, index, isActive, isFocused) =>
            {
                var masterAssetFile = ((List<DefaultAsset>)mastersFileList.list)[index];
                ...
                var masterInkFile = InkLibrary.GetInkFileWithFile(masterAssetFile);
                ...
                var iconRect = new Rect(rect.x, rect.y, 0, 16);
                if (masterInkFile.hasErrors || masterInkFile.hasWarnings) iconRect.width = 20;
                var objectFieldRect = new Rect(iconRect.xMax, rect.y, rect.width - iconRect.width - 80, 16);
                var selectRect = new Rect(objectFieldRect.xMax, rect.y, 80, 16);
                ...
                EditorGUI.EndDisabledGroup();
                if (GUI.Button(selectRect, "Select")) Selection.activeObject = masterAssetFile;
        ...
        private ReorderableList CreateErrorList()
        {
            ...
            reorderableList.drawElementCallback = (rect, index, isActive, isFocused) =>
                DrawLogItem(rect, index, isActive, isFocused, (List<InkCompilerLog>)reorderableList.list);
        ...
        private ReorderableList CreateTodoList()
        {
            ...
            reorderableList.drawHeaderCallback = rect => { EditorGUI.LabelField(rect, "To do"); };
            reorderableList.elementHeight = 26;
            reorderableList.drawElementCallback = (rect, index, isActive, isFocused) =>
                DrawLogItem(rect, index, isActive, isFocused, (List<InkCompilerLog>)reorderableList.list);
            return reorderableList;
        ...
        {
            var logRect = new Rect(rect.x, rect.y, rect.width - 80, 16);
            var locationRect = new Rect(rect.x, rect.y + 16, rect.width - 80, 10);
            var buttonRect = new Rect(logRect.xMax, rect.y, 80, rect.height - 2);
            var log = logsList[index];
            GUI.Label(logRect, log.content);
            GUI.Label(locationRect, "(" + log.relativeFilePath + ":" + log.lineNumber + ")",
                filePathAndLineNumberStyle);
            var openLabel = "Open";
            if (GUI.Button(buttonRect, openLabel)) InkEditorUtils.OpenInEditor(inkFile, log);
        }
        ...
        public static void DrawLayoutInkLine(InkFile inkFile, int lineNumber, string label)
        {
            ...
            var openLabel = "Open" + (lineNumber == -1 ? "" : " (" + lineNumber + ")");
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (inkFile.isIncludeFile)
            {
                ...
                var newCompileAsIfMaster = EditorGUILayout.Toggle(
                    new GUIContent("Should also be Master File",
            ...
            if (inkFile.isMaster)
            {
                ...
                if (inkFile.hasRecursiveIncludeErrorPaths)
                {
                    EditorGUILayout.HelpBox(
                        "A recursive INCLUDE connection exists in this ink file's INCLUDE hierarchy.\nThe offending INCLUDE(s) can be found at the following file(s):\n" +
                ...
                else
                {
                    ...
                    if (inkFile.requiresCompile && GUILayout.Button("Compile")) InkCompiler.CompileInk(inkFile);
            ...
            if (inkFile.isIncludeFile)
            {
                ...
                EditorGUILayout.LabelField(
                    new GUIContent("Include File", InkBrowserIcons.childIconLarge,
                        "This file is included by at least one other file"), EditorStyles.boldLabel);
        ...
        {
            EditorGUILayout.LabelField(new GUIContent("Master File", "This file is a master file and can be compiled"),
            ...
            {
                EditorGUI.BeginChangeCheck();
                var newCompileAutomatically = EditorGUILayout.Toggle(
                    new GUIContent("Compile Automatially",
                        "If true, this file recompiles automatically when any changes are detected."),
                    InkSettings.instance.ShouldCompileInkFileAutomatically(inkFile));
                if (EditorGUI.EndChangeCheck())
                {
...
//				}
        ...
        {
            if (mastersFileList != null && mastersFileList.count > 0) mastersFileList.DoLayoutList();
        }

        private void DrawEditAndCompileDates(InkFile masterInkFile)
        {
            var editAndCompileDateString = "";
            var lastEditDate = inkFile.lastEditDate;
            editAndCompileDateString += "Last edit date " + lastEditDate;
            if (masterInkFile.jsonAsset != null)
            {
                var lastCompileDate = masterInkFile.lastCompileDate;
        ...
        {
            if (includesFileList != null && includesFileList.count > 0) includesFileList.DoLayoutList();
        }
        ...
        private void DrawCompileErrors()
        {
            ...
            if (GUILayout.Button("Report via Email")) Application.OpenURL("mailto:info@inklestudios.com");
            EditorGUILayout.EndHorizontal();
            foreach (var compileError in inkFile.unhandledCompileErrors) GUILayout.TextArea(compileError);
            EditorGUILayout.EndVertical();
        }

        private void DrawErrors()
        {
            if (errorList != null && errorList.count > 0) errorList.DoLayoutList();
        }

        private void DrawWarnings()
        {
            if (warningList != null && warningList.count > 0) warningList.DoLayoutList();
        }

        private void DrawTODOList()
        {
            if (todosList != null && todosList.count > 0) todosList.DoLayoutList();
        }

        private void DrawFileContents()
        {
            var width = EditorGUIUtility.currentViewWidth - 50;
            var height = EditorStyles.wordWrappedLabel.CalcHeight(new GUIContent(cachedTrimmedFileContents), width);
