...
namespace Ink.UnityIntegration
{
	...
	public class InkPlayerWindow : EditorWindow
    {
        ...
        private static bool storyStateValid;
...
//		public class WindowState {
        ...
        private static float timeUntilNextAutomaticChoice;
        private static float timeUntilNextAutomaticContinue;
        ...
        public static Story story
        {
            ...
            private set
            {
                if (_story != null) OnUnsetStory();
                _story = value;
                if (_story != null) OnSetStory();
            }
        }

        public static TextAsset storyStateTextAsset
        {
            get => _storyStateTextAsset;
            set
            {
        ...
        private static Texture playIcon
        {
            get
            {
                if (_playIcon == null) _playIcon = EditorGUIUtility.IconContent("PlayButton").image;
                return _playIcon;
            }
        }
        ...
        private static Texture stopIcon
        {
            get
            {
                if (_stopIcon == null) _stopIcon = EditorGUIUtility.IconContent("PreMatQuad").image;
                return _stopIcon;
            }
        }
        ...
        private static Texture restartIcon
        {
            get
            {
                if (_restartIcon == null) _restartIcon = EditorGUIUtility.IconContent("Refresh").image;
                return _restartIcon;
            }
        }
        ...
        private static Texture saveIcon
        {
            get
            {
                if (_saveIcon == null) _saveIcon = EditorGUIUtility.IconContent("SaveAs").image;
                return _saveIcon;
            }
        }
        ...
        private static Texture clearIcon
        {
            get
            {
                if (_clearIcon == null) _clearIcon = EditorGUIUtility.IconContent("d_Grid.EraserTool").image;
                // d_back
                return _clearIcon;
            }
        }
        ...
        private static Texture undoIcon
        {
            get
            {
                if (_undoIcon == null) _undoIcon = EditorGUIUtility.IconContent("d_back").image;
                // d_back
                // Animation.PrevKey
                return _undoIcon;
            }
        ...
        private static Texture redoIcon
        {
            get
            {
                if (_redoIcon == null) _redoIcon = EditorGUIUtility.IconContent("d_forward").image;
                // d_forward
                // Animation.NextKey
                return _redoIcon;
            }
        ...
        private static Texture warningIcon
        {
            get
            {
                if (_warningIcon == null) _warningIcon = EditorGUIUtility.IconContent("console.warnicon.sml").image;
                return _warningIcon;
            }
        }
        ...
        private static Texture errorIcon
        {
            get
            {
                if (_errorIcon == null) _errorIcon = EditorGUIUtility.IconContent("console.erroricon.sml").image;
                return _errorIcon;
            }
        }
        ...
        private static Texture functionIcon
        {
            get
            {
                if (_functionIcon == null) _functionIcon = EditorGUIUtility.IconContent("d_StyleSheet Icon").image;
        ...
        private static void OnCompleteEvaluateFunction(string functionName, object[] arguments, string textOutput,
            object result)
        {
            var sb = new StringBuilder(functionName);
            if (arguments != null && arguments.Length > 0)
            {
                ...
                var hasTextOutput = textOutput != null && textOutput != string.Empty;
        ...
        {
            var sb = new StringBuilder("ChoosePathString: ");
        ...
        private static void OnUnsetStory()
        {
            ...
            foreach (var observedVariableName in InkPlayerWindowState.Instance.observedVariablesPanelState
                         .restorableObservedVariableNames)
        ...
        private static void OnSetStory()
        {
            ...
            // Reobserve variables
            var variablesToObserve = new List<string>(InkPlayerWindowState.Instance.observedVariablesPanelState
                .restorableObservedVariableNames);
            InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.Clear();
        ...
        private static void PingAutomator()
        {
            ...
            if (story.canContinue && playerOptions.continueAutomatically)
                TryContinue();
            else if (story.currentChoices.Count > 0 && playerOptions.chooseAutomatically) MakeRandomChoice();
        }
        ...
        {
            var sb = new StringBuilder("Story Log\n");
            foreach (var content in storyHistory)
            {
                sb.AppendLine();
        ...
        {
            var item = storyStateHistory.Undo();
        ...
        {
            var item = storyStateHistory.Redo();
            story.state.LoadJson(item.inkStateJSON);
            storyHistory = new List<InkHistoryContentItem>(item.storyHistory);
            RefreshVisibleHistory();
            if (GetShouldAutoScrollOnStoryChange())
        ...
        {
            if (InkPlayerWindowState.Instance.storyPanelState.autoScrollMode ==
                StoryPanelState.AutoScrollMode.Off) return false;
            if (InkPlayerWindowState.Instance.storyPanelState.autoScrollMode ==
                StoryPanelState.AutoScrollMode.Always) return true;
            if (InkPlayerWindowState.Instance.storyPanelState.autoScrollMode ==
                StoryPanelState.AutoScrollMode.WhenAtBottom && storyScrollSnappedToBottom) return true;
            return false;
        }
        ...
        private void DisplayHeader()
        {
            if (attached)
            {
                ...
                GUILayout.Label(new GUIContent(headerTitle.ToString(), attachedStoryIcon,
                ...
                if (GUILayout.Button(new GUIContent("Detach", "Detach from the loaded external story"),
                        EditorStyles.toolbarButton)) Detach();
                ...
                if (CanRestart())
                {
                    ...
                    if (currentStoryJSONLastEditDateTime != updatedStoryJSONLastEditDateTime)
                    {
                        ...
                        EditorGUILayout.HelpBox("Story JSON file has changed. Reload or restart to play updated story.",
                            MessageType.Warning);
                        EditorGUILayout.BeginVertical();
                        if (GUILayout.Button(new GUIContent("Reload", restartIcon,
                                "Reload and restart the current story, which has been updated.")))
                        {
        ...
        private void DisplayStoryControlGroup()
        {
            ...
            else
            {
                ...
                if (GUILayout.Button(new GUIContent("Stop", stopIcon, "Stop the story"),
                        EditorStyles.toolbarButton)) Stop();
                EditorGUI.BeginDisabledGroup(!CanRestart());
                if (GUILayout.Button(new GUIContent("Restart", restartIcon, "Restarts the story"),
                        EditorStyles.toolbarButton)) Restart();
                EditorGUI.EndDisabledGroup();
                EditorGUI.EndDisabledGroup();
            }
            ...
            {
                var profileButtonTitle = new GUIContent("Profile On Start", profilerIcon,
                    "Immediately start the profiler when the story starts.");
                playerParams.profileOnStart = GUILayout.Toggle(playerParams.profileOnStart, profileButtonTitle,
            ...
            if (story != null)
            {
                ...
                if (GUILayout.Button(new GUIContent("Undo", undoIcon, "Undo the last continue or choice"),
                        EditorStyles.toolbarButton, GUILayout.Width(46))) Undo();
                EditorGUI.EndDisabledGroup();
                EditorGUI.BeginDisabledGroup(playerParams.disableUndoHistory || !storyStateHistory.canRedo);
                if (GUILayout.Button(new GUIContent("Redo", redoIcon, "Redo the last continue or choice"),
                        EditorStyles.toolbarButton, GUILayout.Width(46))) Redo();
                EditorGUI.EndDisabledGroup();
            }
            ...
            if (EditorGUI.EndChangeCheck())
                if (!playerParams.disablePlayControls)
                    playerOptions.chooseAutomatically = newChooseAutomatically;
            EditorGUI.EndDisabledGroup();
            EditorGUILayout.LabelField(new GUIContent(timeIntervalIcon, "Time between automatic choices"),
                GUILayout.Width(16));
        ...
        public class InkPlayerWindowState
        {
            ...
            private static void Load()
            {
                ...
                var data = EditorPrefs.GetString(settingsEditorPrefsKey);
                try
                {
...
//		}
        ...
        public static void DrawStoryPropertyField(Story story, InkPlayerParams playerParams, ref bool expanded,
            GUIContent label, bool interactable = false)
        {
            EditorGUILayout.BeginHorizontal();
            ...
            if (EditorApplication.isPlaying)
            {
                if (story != null)
                {
                    ...
                    {
                        if (GUILayout.Button("Detach", GUILayout.Width(80))) Detach();
                    }
                    else
                    {
                        if (GUILayout.Button("Attach", GUILayout.Width(80))) Attach(story, playerParams);
                    }
            ...
            if (expanded)
                if (story != null)
                {
                    ...
                    if (GUILayout.Button("Continue")) story.Continue();
                    EditorGUILayout.EndHorizontal();
        ...
        public static void DrawStoryPropertyField(Rect position, Story story, GUIContent label)
        {
            ...
            if (EditorApplication.isPlaying && story != null /* && story.state != null*/)
            {
                if (attached && InkPlayerWindow.story == story)
                {
                    if (GUI.Button(position, "Detach")) Detach();
                }
                else
                {
                    if (GUI.Button(position, "Attach")) Attach(story);
                }
            }
            else
            {
        ...
        public struct InkPlayerParams
        {
            ...
            public static InkPlayerParams Standard => new();
        ...
        private void DisplayStoryHeader()
        {
            ...
            if (GUILayout.Button(new GUIContent("Clear", clearIcon, "Clears the output"), EditorStyles.toolbarButton,
                    GUILayout.ExpandWidth(false))) ClearStoryHistory();
            GUILayout.Space(12);
            if (GUILayout.Button(new GUIContent("Copy", /*copyToClipboardIcon, */"Copy the output to clipboard"),
                    EditorStyles.toolbarButton, GUILayout.ExpandWidth(false))) CopyStoryHistoryToClipboard();
            ...
            GUILayout.Space(12);
            {
                ...
                InkPlayerWindowState.Instance.storyPanelState.autoScrollMode =
                    (StoryPanelState.AutoScrollMode)EditorGUILayout.EnumPopup(autoScrollGUIContent,
                        InkPlayerWindowState.Instance.storyPanelState.autoScrollMode, EditorStyles.toolbarPopup,
                        GUILayout.Width(130));
                EditorGUIUtility.labelWidth = lw;
            }
            ...
            void DrawVisibilityOptions()
            {
                ...
                EditorGUIUtility.labelWidth = 20;
                var newVisibilityOptions = EditorGUILayout.EnumFlagsField(visibilityOptionsGUIContent,
            ...
            var changed = DrawSearchBar(ref InkPlayerWindowState.Instance.storyPanelState.searchString);
            if (changed)
            {
                RefreshVisibleHistory();
        ...
        private static bool ShouldShowContent(InkHistoryContentItem content,
            DisplayOptions.VisibilityOptions visibilityOpts)
        {
        ...
        private static void RefreshVisibleHistory()
        {
            ...
            var doingSearch = !string.IsNullOrWhiteSpace(InkPlayerWindowState.Instance.storyPanelState.searchString);
        ...
        private void DisplayStoryBody()
        {
            ...
            var showTimestamp = (visibilityOptions & DisplayOptions.VisibilityOptions.TimeStamp) != 0;
            var showTags = (visibilityOptions & DisplayOptions.VisibilityOptions.Tags) != 0;
            ...
            float totalHeight = 0;
            var heights = new float[storyHistory.Count];
            var selectedLineIndex = -1;
            float selectedLineY = -1;

            for (var i = 0; i < visibleHistory.Count; i++)
            {
            ...
            var showScrollToBottomButton = totalHeight - viewportRect.height - (doingAutoscroll
                ? autoscrollTarget
                : InkPlayerWindowState.Instance.storyPanelState.scrollPosition.y) > 100;
            ...
            if (showScrollToBottomButton)
            {
                ...
                if (Event.current.type == EventType.MouseDown && Event.current.button == 0 &&
                    scrollToBottomButtonRect.Contains(Event.current.mousePosition))
                {
                    GUI.FocusControl(null);
                    ScrollToBottom();
                    Event.current.Use();
            ...
            if (newScrollPos != InkPlayerWindowState.Instance.storyPanelState.scrollPosition)
            {
                ...
                storyScrollSnappedToBottom =
                    storyScrollRectBottom - InkPlayerWindowState.Instance.storyPanelState.scrollPosition.y < 0.1f;
            }
                    ...
                    if (Event.current.type == EventType.MouseDown &&
                        ...
                        if (Event.current.button == 0)
                        {
                            ...
                            if (Mathf.Abs(targetY - InkPlayerWindowState.Instance.storyPanelState.scrollPosition.y) >
                                viewportRect.height * 0.225f) ScrollToSelectedLine();
                            Event.current.Use();
                        }
            ...
            if (showScrollToBottomButton) GUI.Box(scrollToBottomButtonRect, "Scroll to bottom", GUI.skin.button);
            ...
            if (Event.current.type == EventType.Layout)
            {
                ...
                if (storyScrollMarkedForSelectedLine != AutoScrollSmoothingMode.NONE && selectedLineIndex != -1)
                {
                    ...
                    autoscrollVelocity = 0;
                    storyScrollSnappedToBottom =
                        storyScrollRectBottom + viewportRect.height * 0.5f - targetPosition < 0.1f;
        ...
        private void DisplayLine(Rect rect, InkHistoryContentItem content)
        {
            ...
            var timeSinceLastWrite = (float)(dateTimeNow - content.time).TotalSeconds;
            var revealTime = 0.8f;
        ...
        private void DisplayChoices()
        {
            ...
            if (story.canContinue)
            {
                ...
                if (GUILayout.Button(new GUIContent("Continue", "Continues once"))) ContinueStory();
            ...
            else if (story.currentChoices.Count > 0)
            {
                ...
                foreach (var choice in story.currentChoices)
                {
                    ...
                    if (GUILayout.Button(new GUIContent(choice.text.Trim(), sb.ToString()))) MakeChoice(choice);
                    GUILayout.EndHorizontal();
                }
        ...
        {
            EditorGUILayout.BeginHorizontal(EditorStyles.toolbar);
            InkPlayerWindowState.Instance.storyStatePanelState.showing =
                EditorGUILayout.Foldout(InkPlayerWindowState.Instance.storyStatePanelState.showing, "Story State",
                    true);
            EditorGUILayout.EndHorizontal();
        ...
        private static void DrawSaveLoad()
        {
            ...
            InkPlayerWindowState.Instance.saveLoadPanelState.showing =
                EditorGUILayout.Foldout(InkPlayerWindowState.Instance.saveLoadPanelState.showing, "Save/Load", true);
            EditorGUILayout.EndHorizontal();
        ...
        private static void DrawSaveLoadPanel()
        {
            ...
            if (GUILayout.Button("Save As...")) SaveStoryState(InkEditorUtils.FormatJson(story.state.ToJson()));
            EditorGUILayout.EndHorizontal();
            ...
            if (GUILayout.Button("Load")) LoadStoryState(storyStateTextAsset.text);
            EditorGUI.EndDisabledGroup();
            EditorGUILayout.EndHorizontal();
        ...
        private void DrawNamedContentHeader()
        {
            ...
            var changed = DrawSearchBar(ref InkPlayerWindowState.Instance.namedContentPanelState.searchString);
        ...
        private void DrawNamedContentItem(Rect rect, string currentPath)
        {
            ...
            if (GUI.Button(new Rect(rect.xMax - 168, rect.y, 80, rect.height),
                    OnDivertLabelOverride == null ? defaultDivertLabel : OnDivertLabelOverride(currentPath)))
            {
        ...
        private void DrawFunctionInput()
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                ...
                InkPlayerWindowState.Instance.functionPanelState.testedFunctionTime = default;
                InkPlayerWindowState.Instance.functionPanelState.functionReturnValue = null;
            }

            functionInputList.DoLayoutList();
            var functionIsValid = functionParams.functionName != string.Empty &&
                                  story.HasFunction(functionParams.functionName);
            ...
            if (GUILayout.Button(new GUIContent(isExternalFunction ? "Execute (External)" : "Execute",
                    "Runs the function")))
            {
                AddToHistory(
                    InkHistoryContentItem.CreateForDebugNote("Execute function '" + functionParams.functionName + "'"));
                string outputContent = null;
                var allInput = new object[functionParams.inputs.Count];
                for (var i = 0; i < functionParams.inputs.Count; i++)
                {
                ...
                if (isExternalFunction)
                {
                    ...
                    OnCompleteEvaluateFunction(functionParams.functionName, allInput, null,
                        InkPlayerWindowState.Instance.functionPanelState.functionReturnValue);
                }
                else
                {
                    InkPlayerWindowState.Instance.functionPanelState.functionReturnValue =
                        story.EvaluateFunction(functionParams.functionName, out outputContent, allInput);
                }
        ...
        {
            var timeSinceLastWrite =
                (float)(dateTimeNow - InkPlayerWindowState.Instance.functionPanelState.testedFunctionTime).TotalSeconds;
            var revealTime = 0.8f;
            var l = Mathf.InverseLerp(revealTime, 0, timeSinceLastWrite);
            var newColor = new Color(GUI.color.r, GUI.color.g, GUI.color.b, 0);
            var color = Color.Lerp(GUI.color, newColor, l);
            ...
            var functionIsValid = functionParams.functionName != string.Empty &&
                                  story.HasFunction(functionParams.functionName);
            if (functionIsValid && functionParams.GetInputHashCode() ==
                InkPlayerWindowState.Instance.functionPanelState.testedFunctionHash)
            {
                GUILayout.BeginVertical(GUI.skin.box);
                if (InkPlayerWindowState.Instance.functionPanelState.functionReturnValue == null)
                    ...
                    EditorGUILayout.Toggle("Output (Bool)",
                        (bool)InkPlayerWindowState.Instance.functionPanelState.functionReturnValue);
        ...
        private void BuildFunctionInputList()
        {
            ...
            functionInputList = new ReorderableList(functionParams.inputs,
                typeof(FunctionPanelState.FunctionParams.FunctionInput), true, true, true, true);
            functionInputList.drawHeaderCallback = rect => { EditorGUI.LabelField(rect, "Inputs"); };
            ...
            functionInputList.drawElementCallback = (rect, index, isActive, isFocused) =>
            {
                ...
                var typeRect = new Rect(rect.x, rect.y, 80, EditorGUIUtility.singleLineHeight);
                input.type =
                    (FunctionPanelState.FunctionParams.FunctionInput.FunctionInputType)EditorGUI.EnumPopup(typeRect,
                        input.type);
                var inputRect = new Rect(rect.x + 90, rect.y, rect.width - 90, EditorGUIUtility.singleLineHeight);
                switch (input.type)
                {
                    case FunctionPanelState.FunctionParams.FunctionInput.FunctionInputType.Float:
        ...
        private void DrawVariablesHeader()
        {
            ...
            var changed = DrawSearchBar(ref InkPlayerWindowState.Instance.variablesPanelState.searchString);
            if (changed)
            {
                RefreshVisibleVariables();
        ...
        private static void RefreshVisibleVariables()
        {
            ...
            if (story == null) return;
            var doingSearch =
                !string.IsNullOrWhiteSpace(InkPlayerWindowState.Instance.variablesPanelState.searchString);
            foreach (var variable in story.variablesState)
            {
                if (doingSearch && !ShouldShowContentWithSearchString(variable,
                        InkPlayerWindowState.Instance.variablesPanelState.searchString)) continue;
        ...
        private void DrawObservableVariable(string variable, ref string variableToChange, ref object newVariableValue)
        {
            if (!SearchStringMatch(variable, InkPlayerWindowState.Instance.variablesPanelState.searchString))
                return;
            EditorGUILayout.BeginHorizontal();
            var variableValue = story.variablesState[variable];
            if (DrawVariableLayout(new GUIContent(variable, variableValue.GetType().Name), variable, ref variableValue,
                    "observable"))
            {
            ...
            {
                if (GUILayout.Button(new GUIContent(observeIcon, "Click to observe this variable, tracking changes"),
                        GUILayout.Width(24)))
                {
        ...
        private static ObservedVariable ObserveVariable(string variableName, bool alsoAddToCache)
        {
            ...
            if (alsoAddToCache)
            {
                InkPlayerWindowState.Instance.observedVariablesPanelState.restorableObservedVariableNames.Add(
                    variableName);
                if (InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.Count !=
                    InkPlayerWindowState.Instance.observedVariablesPanelState.restorableObservedVariableNames.Count)
                {
                    Debug.LogError(InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.Count +
                                   " " + InkPlayerWindowState.Instance.observedVariablesPanelState
                                       .restorableObservedVariableNames.Count);
                    InkPlayerWindowState.Instance.observedVariablesPanelState.restorableObservedVariableNames.Clear();
                    InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.Clear();
                }
            }
        ...
        private static void UnobserveVariable(string variableName, bool alsoRemoveFromCache)
        {
            ...
            {
                InkPlayerWindowState.Instance.observedVariablesPanelState.restorableObservedVariableNames.Remove(
                    variableName);
                if (InkPlayerWindowState.Instance.observedVariablesPanelState.observedVariables.Count !=
                    InkPlayerWindowState.Instance.observedVariablesPanelState.restorableObservedVariableNames.Count)
                {
        ...
        private bool DrawVariableLayout(GUIContent guiContent, string variableName, ref object variableValue,
            string expandedIDModifier)
        {
            ...
            {
                anythingChanged = EditorGUILayoutInkListField(guiContent, (InkList)variableValue,
                    variableName + expandedIDModifier);
            }
        ...
        private void DrawObservedVariablesPanel()
        {
            ...
            foreach (var observedVariable in InkPlayerWindowState.Instance.observedVariablesPanelState
                         .observedVariables)
            {
                var removeVariable = DrawObservedVariable(observedVariable.Value);
                if (removeVariable)
                    allToRemove.Add(observedVariable.Key);
            }

            foreach (var toRemove in allToRemove) UnobserveVariable(toRemove, true);
        }
        ...
        private bool DrawObservedVariable(ObservedVariable observedVariable)
        {
            ...
            if (observedVariable.expanded)
            {
                ...
                foreach (var value in observedVariable.values)
                    DrawVariableLayout(new GUIContent(value.dateTime.ToLongTimeString()), observedVariable.variable,
                        ref value.state, "observed" + value.GetHashCode());
        ...
        {
            get => _currentStoryProfiler != null;
            set
            {
        ...
        private static Profiler _currentStoryProfiler;
        private static Profiler _previousStoryProfiler;
        ...
        private static bool DrawSearchBar(ref string searchString)
        {
            ...
            if (GUILayout.Button("", searchCancelButtonStyle)) searchString = string.Empty;
        ...
        private static bool EditorGUILayoutInkListField(GUIContent guiContent, InkList inkList,
        ...
        private static void EditorGUIInkListField(Rect rect, GUIContent content, InkList inkList, string variableName)
        {
            ...
            if (inkList.Any())
            {
                ...
                {
                    var log = "Log for InkList " + variableName + ":";
                    foreach (var item in inkList)
                        log += item + " / ";
                    Debug.Log(log);
                }
    ...
    public class UndoHistory<T> where T : class
    {
        ...
        public bool canUndo => undoHistory.Count > 0 && undoHistoryIndex > 0;

        public bool canRedo => undoHistory.Count > 0 && undoHistoryIndex < undoHistory.Count - 1;
