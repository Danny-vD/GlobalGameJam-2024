...
namespace Ink.Runtime
{
    public class NativeFunctionCall : Object
    {
        ...
        {
            if (_prototype) return _prototype.Call(parameters);

            if (numberOfParameters != parameters.Count) throw new Exception("Unexpected number of parameters");
            ...
            if (coercedType == ValueType.List) return Call<InkList>(coercedParams);
        ...
        private Value Call<T>(List<Value> parametersOfSingleType)
        {
            ...
            throw new Exception(
                "Unexpected number of parameters to NativeFunctionCall: " + parametersOfSingleType.Count);
        }
        ...
        private Value CallListIncrementOperation(List<Object> listIntParams)
        {
            ...
            foreach (var listItemWithValue in listVal.value)
            {
                ...
                // Return value unknown until it's evaluated
                var targetInt = (int)intOp(listItemValue, intVal.value);
        ...
        private List<Value> CoerceValuesToSingleType(List<Object> parametersIn)
        {
            ...
            {
                var val = (Value)obj;
                if (val.valueType > valType) valType = val.valueType;
            ...
            if (valType == ValueType.List)
                foreach (Value val in parametersIn)
                    ...
                    {
                        var intVal = (int)val.valueObject;
        ...
        private static void GenerateNativeFunctionsIfNecessary()
        {
            if (_nativeFunctions == null)
            {
                ...
                BinaryOp<Path> divertTargetsEqual = (d1, d2) => { return d1.Equals(d2); };
                BinaryOp<Path> divertTargetsNotEqual = (d1, d2) => { return !d1.Equals(d2); };
        ...
        {
            if (_operationFuncs == null) _operationFuncs = new Dictionary<ValueType, object>();
