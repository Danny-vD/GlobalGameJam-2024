...
namespace Ink.Runtime
{
    ...
    public class Story : Object
    {
        ...
        /// <summary>
        ///     Is the default flow currently active? By definition, will also return true if not using multi-flow functionality -
        ///     see SwitchFlow
        /// </summary>
        public bool currentFlowIsDefaultFlow => state.currentFlowIsDefaultFlow;

        /// <summary>
        ///     Names of currently alive flows (not including the default flow)
        /// </summary>

        public List<string> aliveFlowNames => state.aliveFlowNames;
            ...
            int currTagCount)
        {
            ...
            var newlineStillExists = currText.Length >= prevText.Length && prevText.Length > 0 &&
        ...
        private Choice ProcessChoice(ChoicePoint choicePoint)
        {
            ...
            if (choicePoint.hasChoiceOnlyContent) choiceOnlyText = PopChoiceStringAndTags(ref tags);

            if (choicePoint.hasStartContent) startText = PopChoiceStringAndTags(ref tags);
                    ...
                    case ControlCommand.CommandType.EndTag:
                    {
                        ...
                        if (state.inStringEvaluation)
                        {
                            ...
                            for (var i = state.outputStream.Count - 1; i >= 0; --i)
                            {
                                ...
                                if (command != null)
                                {
                                    if (command.commandType == ControlCommand.CommandType.BeginTag) break;
                            ...
                            var sb = new StringBuilder();
                            foreach (StringValue strVal in contentStackForTag) sb.Append(strVal.value);
            ...
            foreach (var c in flowContainer.content)
            {
                ...
                if (command != null)
                {
                    if (command.commandType == ControlCommand.CommandType.BeginTag)
                        inTag = true;
                    else if (command.commandType == ControlCommand.CommandType.EndTag) inTag = false;
                }
