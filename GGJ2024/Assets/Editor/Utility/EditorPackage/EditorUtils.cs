...
namespace Utility.EditorPackage
{
    ...
    {
        private static readonly Color grey = new(0.4f, 0.4f, 0.4f);
        ...
        /// <summary>
        ///     Will draw an adjustable-size list with custom labels and a texture and calls elementAction(index, array[i]) for
        ///     every element
        /// </summary>
        ...
        )
        {
            if (IsFoldOut(ref isFoldOut, propertyIcon, propertyName))
            {
                ...
                for (var i = 0; i < size; i++) elementAction(i, array.GetArrayElementAtIndex(i));
        ...
        )
        {
            if (IsFoldOut(ref isFoldOut, propertyName))
            {
                ...
                for (var i = 0; i < array.arraySize; i++) elementAction(i, array.GetArrayElementAtIndex(i));
        ...
        public static string GetTypeString(SerializedProperty property)
        {
            if (property == null)
                throw new ArgumentNullException(nameof(property),
                    "SerializedProperty is null, check if the type has serializable fields");
        ...
        ///     [OPTIONAL] A string that will be returned if no other string is appropriate ('{index}' will
        ///     be replaced with the value of the index parameter)
        /// </param>
        ...
        public static string GetValueString(SerializedProperty serializedProperty, int index = 0,
            string defaultText = "Element {index}", Type propertyType = null)
        {
            if (defaultText.Contains("{index}")) defaultText = defaultText.Replace("{index}", index.ToString());
            ...
            switch (serializedProperty.propertyType)
            {
                ...
                case SerializedPropertyType.Enum:
                    var enumNames = propertyType != null ? Enum.GetNames(propertyType) : serializedProperty.enumNames;
                ...
                case SerializedPropertyType.Character:
                    var value = (char)serializedProperty.intValue;
		...
		/// <summary>///     Calls elementAction(index, @struct) for every element in the array/// </summary>
		public static void DrawKeyValueArray(SerializedProperty array, Action<int, SerializedProperty> elementAction)
		{
			...
			{
				var @struct = array.GetArrayElementAtIndex(i);
		...
		/// <summary>///     Calls elementAction(index, key, value) for every element in the array/// </summary>
		...
		)
		{
			...
			{
				var @struct = array.GetArrayElementAtIndex(i);
				var key = @struct.FindPropertyRelative(keyName);
				var value = @struct.FindPropertyRelative(valueName);
		...
		/// <summary>///     Will create a foldout label with a texture for every value in the enum and call elementAction(index, @struct) for///     every element/// </summary>
			...
			where TEnum : struct, Enum
		{
			...
			{
				var enumString = ConvertIntToEnum<TEnum>(@struct.FindPropertyRelative(keyName).enumValueIndex)
					.ToString()
		...
		/// <summary>///     Will create a foldout label with a texture for every value in the enum and call elementAction(index, key, value)///     for every element/// </summary>
			...
			where TEnum : struct, Enum
		{
			...
			{
				var enumString = ConvertIntToEnum<TEnum>(key.enumValueIndex).ToString().ReplaceUnderscoreWithSpace();
		...
		/// <summary>///     Will create a foldout label for every value in the enum and call elementAction(index, @struct) for every element/// </summary>
		...
		/// <summary>///     Will create a foldout label for every value in the enum and call elementAction(index, key, value) for every element/// </summary>
		public static void DrawFoldoutKeyValueArray<TEnum>(SerializedProperty array, string keyName, string valueName,bool[] foldouts,
			Action<int, SerializedProperty, SerializedProperty> elementAction)
			where TEnum : struct, Enum
		{
		...
		/// <summary>///     Will create a foldout label with a texture for every value in the enum and draw every value with a specified label/// </summary>
		public static void DrawFoldoutKeyValueArray<TEnum>(SerializedProperty array, string keyName, string valueName, bool[] foldouts,
		...
		/// <summary>///     Will create a foldout label for every value in the enum and draw every value with a specified label/// </summary>
		public static void DrawFoldoutKeyValueArray<TEnum>(SerializedProperty array, string keyName, string valueName, bool[] foldouts,
