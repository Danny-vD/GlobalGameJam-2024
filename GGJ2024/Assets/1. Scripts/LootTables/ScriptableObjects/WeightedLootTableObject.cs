...
namespace LootTables.ScriptableObjects
{
    ...
    {
        [SerializeField] private SerializableDictionary<TLootType, long> weightedLootTable;
        ...
        {
            if (!weightedLootTable.Contains(pair)) return false;
        ...
        {
            return
                TryRemove(new KeyValuePair<TLootType, long>(loot,
                    0)); // Weight does not matter, SerializableKeyValuePairs are equal by their Key only
        }
    }
}