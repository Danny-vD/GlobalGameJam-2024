...
namespace APIs.DiscordIntegrationPackage.RichPresence.Components.DynamicData
{
    ...
    {
        [SerializeField] private DynamicDataLocation dynamicDataLocation = DynamicDataLocation.State;
        ...
        public void UpdatePresence()
        {
            ...
            if (showDetails)
                activity.Details = dynamicDataLocation.HasFlag(DynamicDataLocation.Details)
                    ? detailsWriter.UpdateText(GetData())
                    : details;

            if (showState)
                activity.State = dynamicDataLocation.HasFlag(DynamicDataLocation.State)
                    ? stateWriter.UpdateText(GetData())
                    : state;
